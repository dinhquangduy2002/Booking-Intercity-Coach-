...
const ListOwner = () => {
    ...
    const accountDetail = (acc) => {
        setAccount(acc);
        getTripsByAccountId(acc.id);
        calculateRevenue(acc.id);
    }
    const getTripsByAccountId = (ownerId) => {
        TripByIdService.findByOwnerId(ownerId , currentPageMd - 1)
            .then((trips) => {
                console.log(trips.content);
                setTrips(trips.content);
                setTotalPagesMd(trips.totalPages)
                setLgShow(true);
            })
            .catch((err) => {
                console.log(err);
    ...
    const getRevenue = (trip) => {
        let revenue = 0;
        for (let i = 0; i < trip.length; i++) {
            revenue += trip[i].revenue;
        }
        return revenue;
    };

    const calculateRevenue = (id) => {
        tripByIdService.getRevenueByOwnerId(id).then((response) => {
    ...
    }, [currentPage, currentPageMd, nameSearch, revenue, trips, status, isLoad])
    useEffect(() => {
            getTripsByAccountId(account.id , currentPageMd );
    },[currentPageMd])
    ...
    return (
        <div className="col-9">
            <h3 className="text-uppercase text-center mb-5">Danh sách quản lý</h3>
            ...
            >
                ...
                <Modal.Body>
                    <div className="row">
                        ...
                        <div className="row">
                            <h2 className="text-md-center">Danh sách chuyến đi</h2>
                            <Table hover>
                                <thead>
                                <tr align="center" style={{fontSize: '20px'}}>
                                    <th>STT</th>
                                    <th>Chuyến đi</th>
                                    <th>Số ghế đã thuê</th>
                                    <th>Giá thuê</th>
                                    <th>Ngày đi</th>
                                </tr>
                                </thead>
                                    ...
                                    trips.map((h, index) => {
                                        return (
                                            <tr key={index} align="center">
                                                ...
                                                <td>
                                                    <Link to={`/trip-detail/${h.id}`} className="nav-link">
                                                        <h5>{h.name}</h5>
                                                    </Link>
                                                </td>
                                                <td>{h.seatBooking}</td>
