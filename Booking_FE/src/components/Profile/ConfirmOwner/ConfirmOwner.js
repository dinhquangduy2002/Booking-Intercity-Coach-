...
const ConfirmOwner = () => {
    ...
    const handleAgree = (value) => {
        let data = {...value, status: "Chờ nhận vé"};
        Swal.fire({
            title: `Bạn đồng để ${value.lastname} ${value.firstname} làm chủ chứ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                AccountService.agreeRegister(data).then((response) => {
                    ...
                    setLoad(!load);
                    handleSendNotify(account, value.account.id, 'Admin đã đồng ý cho bạn làm chủ phòng', 'profile/trips-owner');
                }).catch(function (err) {
                    console.log(err);
                })
            }
